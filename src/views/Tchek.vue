<template>
    ...
    <div class="flex justify-center pt-2 bg-white">
        <button class="p-3 text-white bg-green-600 rounded-md" :class="[!isValide ? 'block' : 'hidden']" @click="valider">Valider</button>
        <a class="p-3 text-white bg-green-600 rounded-md" :class="[isValide ? 'block' : 'hidden']" :href="rapportSrc">Cliquez pour voir le rapport</a>
    </div>
</template>
